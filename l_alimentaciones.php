            ...
            <div class="card-body">
                <div class="search-section">
                    <form class="search-form" action="l_alimentaciones.php" method="GET">
                        <input type="text" id="buscar-alimentacion" name="buscar" placeholder="Buscar por animal o documento..." value="<?php echo isset($_GET['buscar']) ? htmlspecialchars($_GET['buscar']) : ''; ?>">
    ...
    <script>
        $(document).ready(function() {
            new BuscadorRealtime({
                inputSelector: '#buscar-alimentacion',
                tableSelector: '#tabla-alimentaciones tbody',
                emptyStateSelector: '.empty-state',
                apiEndpoint: 'controllers/alimentaciones/op_buscar.php',
                        ...
                        <tr>
                            ...
                            <td>
                                <form id="form-eliminar-${$('<div>').text(alimentacion.id).html()}" action="controllers/alimentaciones/op_eliminar.php" method="POST" class="form-inline">
                                    <input type="hidden" name="id" value="${$('<div>').text(alimentacion.id).html()}">
                                    <button type="button" class="btn btn-delete btn-swal-eliminar" data-id="${$('<div>').text(alimentacion.id).html()}" data-nombre="${$('<div>').text(alimentacion.id || 'este registro').html()}">
                                        <i class="fas fa-trash-alt"></i> Eliminar
                                    </button>
                                </form>
                            </td>
