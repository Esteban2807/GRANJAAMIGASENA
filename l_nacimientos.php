...
$nacs = new nacimientos();
if (isset($_GET['buscar']) && trim($_GET['buscar']) !== '') {
    $res = $nacs->buscar($_GET['buscar']);
} else {
    $res = $nacs->listar();
}
?>
    ...
    <main class="container">
        <div class="content-card">
            <div class="card-header">
                <h1 class="card-title"><i class="fas fa-baby-carriage"></i> Nacimientos</h1>
                <?php if (in_array($rolId, [1,2])): ?>
                <a href="cr_nacimiento.php" class="btn-create"><i class="fas fa-plus-circle"></i> Crear Nuevo</a>
                <?php endif; ?>
            </div>
            <div class="card-body">
                        ...
                        <tbody>
                            <?php foreach ($res as $registro): ?>
                                <tr>
                                    ...
                                    <td><?php echo htmlspecialchars($registro['sexo']); ?></td>
                                    <td><?php echo htmlspecialchars($registro['vigor']); ?></td>
                                    ...
                                    <?php if (in_array($rolId, [1,2])): ?>
                                    <td>
                                        <form id="form-eliminar-<?php echo $registro['id']; ?>" action="controllers/nacimientos/op_eliminar.php" method="POST" class="form-inline">
                                            <input type="hidden" name="id" value="<?php echo htmlspecialchars($registro['id']); ?>">
